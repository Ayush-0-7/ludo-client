...
import { BASE_PADS, TRACK_PATH, HOME_LANE_PATHS, BOARD_SIZE, CELL_SIZE, HOME_ENTRY_INDEX, ENTRY_INDEX, TRACK_LEN } from '../config/constants';
...
const calculatePath = (player, startToken, endToken, diceValue) => {
    ...
    if (startToken.state === 'base' || endToken.state === 'done') {
        path.push(endToken); // Single step for entering or finishing
        return path;
    }
    ...
    // Ensure the final step matches the server's end state exactly
    if (path.length > 0 && (path[path.length - 1].pos !== endToken.pos || path[path.length - 1].state !== endToken.state)) {
        path.push(endToken);
    }
    return path;
};

export default function GameScreen({ gameState, socket }) {
    const [animatedPlayers, setAnimatedPlayers] = useState(null);
    const [isAnimating, setIsAnimating] = useState(false);
    const previousGameState = usePrevious(gameState);

    // Initialize or sync animatedPlayers with gameState
    useEffect(() => {
        if (!animatedPlayers) {
            setAnimatedPlayers(JSON.parse(JSON.stringify(gameState.players)));
        }
    }, [gameState, animatedPlayers]);

    // Main effect to detect and run animations
    useEffect(() => {
        if (!previousGameState || !animatedPlayers || !gameState) return;
        ...
        for (const [pIdx, player] of gameState.players.entries()) {
            for (const [tIdx, token] of player.tokens.entries()) {
                ...
                if (prevToken && JSON.stringify(prevToken) !== JSON.stringify(token)) {
                    movedTokenInfo = { pIdx, tIdx, startToken: prevToken, endToken: token };
                    break;
                }
            }
            ...
            const runAnimation = async () => {
                ...
                if (path.length === 1) {
                    ...
                    for (const step of path) {
                        ...
                        await new Promise(res => setTimeout(res, 250)); // Speed of each step
                    }
                }

                // Check for captures after animation
                const prevBaseCount = previousGameState.players.reduce((sum, p) => sum + p.tokens.filter(t => t.state === 'base').length, 0);
                const currBaseCount = gameState.players.reduce((sum, p) => sum + p.tokens.filter(t => t.state === 'base').length, 0);
                if(currBaseCount > prevBaseCount && startToken.state !== 'base'){
                    SoundManager.playTokenCapture();
                }

                // Sync with final server state and end animation
                setAnimatedPlayers(JSON.parse(JSON.stringify(gameState.players)));
    ...
    const isMyTurn = myPlayer && myPlayer.id === activePlayer.id;

    const legal = useMemo(() => {
        if (!isMyTurn || gameState.diceValue == null) return [];
        return legalMovesForPlayer(activePlayer, gameState.players, gameState.diceValue);
    ...
    const executeMove = (mv) => {
        if(isAnimating) return;
        if (legal.find(legalMove => JSON.stringify(legalMove) === JSON.stringify(mv))) {
            socket.emit('makeMove', { roomId: gameState.roomId, move: mv });
        }
    };
    ...
    const tokenCoord = (playerColor, token, tokenIndex) => {
        ...
        else if (token.state === "home" || token.state === 'done') ({ r, c } = HOME_LANE_PATHS[playerColor][token.pos]);
        else return { x: -100, y: -100 };
        return { x: (c + 0.5) * CELL_SIZE, y: (r + 0.5) * CELL_SIZE };
    };

    const selectableTokens = useMemo(() => new Set(legal.map(mv => mv.token)), [legal]);
  ...
  <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row gap-6 sm:gap-8 items-center">
    ...
    {/* --- Controller --- */}
    <div className="flex-[1] bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl 
                    p-4 sm:p-6 flex flex-col gap-3 sm:gap-5 w-full md:w-auto text-sm sm:text-base">
