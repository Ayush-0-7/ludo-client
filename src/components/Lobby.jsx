...
export default function Lobby({ gameState, socket, userId, onLeave }) {
  ...
  return (
    <div className="w-full max-w-lg mx-auto p-8 bg-white rounded-2xl shadow-2xl text-center">
      ...
      <div className="bg-neutral-200 p-3 rounded-lg text-4xl font-mono tracking-widest mb-6 select-all">
        {gameState.roomId}
      </div>
      ...
      <div className="space-y-3 mb-8">
        {gameState.players.map((player) => (
          <div
            key={player.userId}
            className={`flex items-center gap-3 p-3 bg-neutral-100 rounded-lg transition-opacity ${
              player.disconnected ? "opacity-50" : ""
            }`}
          >
            <span
              className={`w-6 h-6 rounded-full ${
                COLOR_CLASSES[player.color].bg
              }`}
            ></span>
            <span className="font-bold text-lg">{player.name}</span>
            {player.userId === gameState.hostId && (
              <span className="text-sm font-semibold text-yellow-600 ml-auto">
                (Host)
              </span>
            )}
          </div>
